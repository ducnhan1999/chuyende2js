...
function Del()
    ...
    function Show()
    {
        for(i=0;i<dssv.length;i++)
            {
                document.getElementById("dssv").innerHTML += "<input type='checkbox' name='delete'>" + dssv[i].mssv + " " + dssv[i].hoten + " " + dssv[i].email + " " + "<button id='btnEdit" + (len - 1) + "' onclick='Edit(" + (len - 1) + ")''>EDIT</button>" + "<br/>";
            }
    }
function Save(id)
{
    ...
    dssv[id].msv = document.getElementById("ma-sinh-vien").value;
    dssv[id].name = document.getElementById("ho-va-ten").value;
